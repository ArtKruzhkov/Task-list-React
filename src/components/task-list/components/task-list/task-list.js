...
const TaskList = () => {
    ...
    const [hoveredTaskId, setHoveredTaskId] = useState(null);
    const [hoveredDeleteButtonId, setHoveredDeleteButtonId] = useState(null); // Состояние для кнопки удаления
            ...
            <List>
                {tasks.map((task, index) => (
                    <ListItem
                        ...
                        key={task.id}
                        ref={index === tasks.length - 1 ? lastTaskRef : null}
                        onMouseEnter={() => setHoveredTaskId(task.id)} // Наводим на задачу
                        onMouseLeave={() => setHoveredTaskId(null)} // Убираем наведение с задачи
                    >
                        <div className={styles.taskItem}>
                            <ListItemText
                                primary={task.description}
                                className={styles.listItemText}
                            />
                            {hoveredTaskId === task.id && hoveredDeleteButtonId !== task.id && ( // Показываем кнопку редактирования только если не наведены на кнопку удаления
                                <IconButton
                                    edge="start"
                                    onClick={() => handleEditOpen(task)}
                                    className={styles.editButton}
                        ...
                        <div
                            ...
                            onMouseEnter={() => setHoveredDeleteButtonId(task.id)} // Наводим на кнопку удаления
                            onMouseLeave={() => setHoveredDeleteButtonId(null)} // Убираем наведение с кнопки удаления
                        >
                            <IconButton
                                edge="end"
                                onClick={() => handleOpenDialog(task.id)}
